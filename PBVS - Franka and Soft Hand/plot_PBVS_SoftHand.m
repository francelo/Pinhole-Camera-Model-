...
proj = out.proj;
control = out.control;
time = linspace(0,length(states)*st,length(states));
...
%     quiver3([out.pose(i,1);out.pose(i,1);out.pose(i,1)],[out.pose(i,2);out.pose(i,2);out.pose(i,2)],[out.pose(i,3);out.pose(i,3);out.pose(i,3)],ee_rot(1,:)'/4,ee_rot(2,:)'/4,ee_rot(3,:)'/4) % ee frame
   ...
   % annotation('textbox', [0, 0.87, 0.115, 0], 'string', '')
    ...
    plot(time(1:i), out.error(1:i, :), 'LineWidth', 0.8)
    title('Error')
    ...
    plot(time(1:i), out.states(1:i, :),'LineWidth', 0.8)
    legend("camera flag", 'control flag', 'control switch', 'grasp flag')
    title('States')
...
%%
% 
video = VideoWriter('Simulation_SoftHand.avi', 'Uncompressed AVI');
video.FrameRate = 20;
open(video);
writeVideo(video, F);
close(video);
...
axis([0 length(error)*st -0.6 0.6]);
legend('err x', 'err y', 'err z', 'err \phi', 'err \theta', 'err \psi')
grid on

% states 
figure()
plot(time, states, 'LineWidth', 1.5)
axis([0 length(states)*st -0.05 1.05]);
legend('camera flag', 'control flag', 'control switch', 'grasp flag')
grid on
